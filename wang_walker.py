...
class DeepQNetwork:
    def __init__(self, sess, a_dim, s_dim, args):
        ...
        #Befehl für aktualisierung von Keras
        ...
        self.a =  tf.compat.v1.placeholder(tf.float32, [None,], name='action')  # input Action
        ...
        with  tf.compat.v1.variable_scope("Q_at_a"):
            ...
            one_hot_actions = tf.one_hot(tf.cast(self.a,tf.int32), self.a_dim) # shape (batch_size, a_dim)
            q_all = tf.multiply(self.q, one_hot_actions) # shape (batch_size, a_dim)
            self.q_at_a =  tf.compat.v1.reduce_sum(q_all, axis=1) # shape (batch_size,)
        ...
        with  tf.compat.v1.variable_scope("soft_update"):
            ...
            self.update_op = [tf.compat.v1.assign(t, (1 - TAU)*t + TAU*e) for t, e in zip(self.nn_params_, self.nn_params)]
    ...
    def choose_action(self, sess, observation):
        ...
        if np.random.uniform() <= explore_p:
            ...
            action0 = np.random.randint(-1,1)
            action1 = np.random.randint(-1,1)
            action2 = np.random.randint(-1,1)
            action3 = np.random.randint(-1,1)
            action=np.array([action0,action1,action2,action3])
        else:
            ...
            action = np.array(np.argmax(Qs[0]),np.argmax(Qs[1]),np.argmax(Qs[2]),np.argmax(Qs[3]))
        return action
...
for ep in range(num_ep):
    ...
    rewards_only_list.append(ep_reward)
    ...
    print("episode: {}/{}, steps: {}, explore_prob: {:.2f}, total reward: {}, average reward: {:.2f}, state: {}".\
              format(ep, num_ep, t_step, agent.epsilon, ep_reward,np.mean(rewards_only_list),state[0]))
