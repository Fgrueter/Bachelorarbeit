    ...
    while True: # Each actual simulation run instance
        ...
        while True: # Each timestep within said simulation...
           ...
           # frames.append(Image.fromarray(env.render(mode='rgb_array')))  # save each frames
            ...
            thr = -110.0
            if(run>1000):
                if any(my_sma(scores, 100) >= -110.0):
                    print("true")
                    ep_solve = np.argwhere(my_sma(scores, N) >= thr).ravel()[0] -N # find where sma > thr   
                    print("episodes before solving: {},{}".format(ep_solve,np.mean(scores)))
   
                rewards,eps=np.array(scores_eps).T  
                fig = plt.figure()
                # plot reward v.s. episode
                plt.plot(eps, rewards)
                plt.xlabel('episode')
                plt.ylabel('reward')
                fig.savefig("reward_plot_{}.jpeg".format(ENV_NAME))
               # plt.show() 
                env.close()   
                sys.exit()   
